...
public class UI_ManagerServicesSkeleton {
	...
	public void outputOperation(
			final be.ac.fundp.OutputOperation outputOperation) {
		Runnable r1 = new Runnable() {
			public void run() {
				clientDispatcher.dispatchOutputOp(outputOperation.getId(),
						outputOperation.getData());
			}
		};
	...
	public be.ac.fundp.InputOperationResponse inputOperation(
		...
		InputOperationResponse i = clientDispatcher
				.dispatchInputOp(inputOperation.getId());
		return i;
	}
	...
	public be.ac.fundp.SelectionOperationResponse selectionOperation(
		...
		SelectionOperationResponse i = clientDispatcher.dispatchSelectionOp(
				selectionOperation.getId(), selectionOperation.getData());
