...
public class DataInputUIPropertySection extends BPELPropertySection {
	...
	public class SetVariableCommand extends SetCommand {
		...
		@Override
		public Object get() {
			return ((DataInputUI)fTarget).getInputVariable();
		}
		@Override
		public void set(Object o) {
			((DataInputUI)fTarget).setInputVariable((Variable)o);
		}
	}
	...
	protected void createClient(Composite parent) {
		...
		Label variableLabel = fWidgetFactory.createLabel(composite, Messages.UIBPELEditor_Property_InputVar);
				//"Variable input:"); 
		...
		variableBrowseButton = fWidgetFactory.createButton(composite, 
				Messages.FaultThrowNameSection_Browse_1, SWT.PUSH); 

		data = new FlatFormData();
		data.left = new FlatFormAttachment(0, BPELUtil.calculateLabelWidth(variableLabel,
				STANDARD_LABEL_WIDTH_SM));
		data.right = new FlatFormAttachment(variableBrowseButton, -IDetailsAreaConstants.HSPACE);
		data.top = new FlatFormAttachment(0, 0);
		data.height = FigureUtilities.getTextExtents(variableBrowseButton.getText(),
				variableBrowseButton.getFont()).height + 4;
	...
	public void updateVariableWidgets() {
		if (getActivity() != null ) {
			Variable variable = getActivity().getInputVariable();
			if (variable!=null) {
