...
public class BPEL_UI_Serializer implements BPELActivitySerializer {
	...
	public Element marshallInternalElement(QName qName, Node parentNode,
		...
		if (element instanceof UserRole) {
			UserRole sa = (UserRole)element;
			Document document = parentNode.getOwnerDocument();
			//it works?
			saElement = (Element) userRole2XML(sa,document,qName,process,bpelWriter);
			return saElement;
	...
	private Element userRole2XML(UserRole sa, Document document, QName qName,
			Process process, BPELWriter bpelWriter) {
		Element saElement = document.createElementNS(qName.getNamespaceURI(),
				BPEL_UI_Constants.ND_USER_ROLE);
		saElement.setPrefix(BPEL_UI_Utils.addNamespace(process));
		
		if (sa.getId() != null) {
			saElement.setAttribute("id", sa.getId());
		}
		return saElement;
	}
